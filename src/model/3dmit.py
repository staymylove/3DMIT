...
VISION_TAGS = {
    ...
    "sov": {"image": "<Img>", "pcl": "The whole scene information:"},
    "eov": {"image": "</Img>", "pcl": ".\n"},
    "sobj": {"image": "</Img>", "pcl": "The information of all the objects in the scene:"}, 
    "eobj": {"image": "</Img>", "pcl": ".\n"}
}
...
class 3DMITPEFT(nn.Module):
    """LoRA for tuning"""

    def __init__(self, **args):
        super(3DMITPEFT, self).__init__()
        self.args = args
        self.input_dim =1024
    ...
    def encode_scnanet_object(self,input_path):
        ...
        if 'scannet' in input_path[0]:
            # obj_path= './src/feature/scannet_ulip2_feats.pt'    #1024 need scene0001_00_000                                                              #512  need  scene0001_00_00
            obj_path='./src/feature/scannet_uni3d_feats.pt'
            ...
            with open(attr_path, 'r') as file:
                attr = json.load(file)
            for i in object_all.keys():   #scene0000_00_001
                j=0
                if i.startswith(target_id):  #scene0001_00_01 start with  scene0001_00
                    #import ipdb; ipdb.set_trace()
                    batch_size = 1
                    #object_all[i] -> (512)
            ...
            obj_feat=torch.cat(ls, dim=1)  #[1,num_obj,512]
            obj_feat.to(self.device)
            return obj_feat

        if '3rscan' in input_path[0]:
            obj_path= './src/feature/3rscan_uni3d_feats_1024.pt'
            attr_path= './src/feature/3rscan_attributes.json' 

            pattern = re.compile(r'scene\d{4}_\d{2}')

            target_id = pattern.search(input_path[0]).group()
            
            object_all=torch.load(obj_path)  #scene0001_00_000
            #object_all=dict(sorted(object_all.items(), key=lambda item: item[0][-3:]))
            ls=[]
            ...
            for i in object_all.keys():
                ...
                if i.startswith(target_id):
                    ...
                    #print(pos_emb.shape)          # # torch.Size([b, t, 512])
                    # feat_all = self.input_norm(feat + size_color_emb + pos_emb).to(self.device)
                    feat_all = self.input_norm(feat).to(self.device)
                    obj_feat = self.scene_proj(feat_all).to(self.device)  # torch.Size([b, t, 4096])
    ...
    #         obj_path='./src/feature/scannet_uni3d_feats.pt'
    #         attr_path= './src/feature/scannet_attributes.json' 
    #         # input_path =['./src/data/3D_Instruct/scannet_pcls/scene0614_00_vert.npy'
    #         #               ./src/data/3D_Instruct/scannet_pcls/scene0000_00_vert.npy]
    #         target_id=input_path[0][64:76] # 'scene0001_00
    #         object_all=torch.load(obj_path)
    #         # print(a.keys())
    #         ls=[]
    ...
    #                 pos_dim3.unsqueeze(0) #tensor([1,512])
    #                 pos_dim3=torch.unsqueeze(pos_dim3, 0).expand(batch_size, -1, -1)  
    #                 pos_emb = self.pos_embedding(pos_dim3, self.input_dim).permute(0, 2, 1).to(self.device)  # torch.Size([b, t, 512])
    #                 #print(pos_emb.shape)          # # torch.Size([b, t, 512])
    #                 feat_all = self.input_norm(feat + size_color_emb + pos_emb).to(self.device)
    #                 obj_feat = self.scene_proj(feat_all).to(self.device)  # torch.Size([b, t, 4096])
    ...
    #                 if j!= 0:
    #                     index= f"; obj{j:02} "
    #                 index_token=self.llama_tokenizer(index, return_tensors="pt", add_special_tokens=False).to(self.device) 
    #                 index_token_ids = index_token.input_ids.expand(batch_size, -1)  # bsz x s1
    #                 index_token_embeds = self.llama_model.model.model.embed_tokens(index_token_ids)
    #                 print('indexshape',index_token_embeds.shape)
    #                 ls.append(index_token_embeds)
    #                 ls.append(obj_feat)
    #             j+=1

    #         obj_feat=torch.cat(ls, dim=1)  #[1,num_obj*6,4096]
    #         obj_feat.to(self.device)
    #         return obj_feat
    ...
    def encode_pcl(self, pcl_paths):
        ...
        #             self.llama_model.dtype
        #         )  # bsz*num vision token x 1024        
        image_embeds = torch.load('./src/pcl_features/scannet_features_from_epcl/'+id+'.pt').to(
                    self.llama_model.dtype
                ).to(self.device)
        # image_embeds=image_embeds.reshape(1,32,4096*2)
        inputs_llama = self.llama_proj(image_embeds).reshape(
            1, 256, self.llama_model.config.hidden_size
        )  # bsz x num_vision_token x llama_size
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(
            self.device
        )  # bsz x 1/256
    ...
    def prompt_wrap(
        self, img_embeds, obj_feat, input_ids, target_ids, attention_mask, use_system, task_type
    ):
        """
        ...
        inputs_embeds = torch.cat(
            [bos_embeds, p_before_embeds, img_embeds,p_middle_embeds, obj_feat, p_after_embeds], dim=1
        )  # bsz x (1+ s1+NumToken+obj+s2) x embed_dim  torch.Size([1, 548, 4096])
           #<obj1>:  <obj2>:  ..... <obj44>:
        # make target ids for prefix part
        empty_targets = (
            torch.ones(
                [batch_size, 1 + p_before_embeds.size()[1] + obj_feat.size()[1]+ p_middle_embeds.size()[1]+ self.num_vision_token],
                dtype=torch.long,
            )
            .to(self.device)
        ...
        #     [atts_bos, p_before_attn_mask,atts_obj, p_middle_attn_mask, atts_img, attention_mask], dim=1
        # )
        attention_mask=torch.ones([batch_size, inputs_embeds.size()[1]], dtype=torch.long).to(
            self.device
        )  
        assert (
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        inputs_embeds = torch.cat(
            [bos_embeds, p_before_embeds, feature_embeds, p_middle_embeds, obj_feat, p_after_embeds], dim=1
