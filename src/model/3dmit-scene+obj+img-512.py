...
from .utils.position_embedding import PositionEmbeddingCoordsSine
ImageFile.LOAD_TRUNCATED_IMAGES = True


VISION_TAGS = {
    "pos": {"image": "<image>", "pcl": "<pcl>"},
    "sov": {"image": "<Img>", "pcl": "The whole scene infomation:"},
    "eov": {"image": "</Img>", "pcl": ".\n"},
    "sobj": {"image": "</Img>", "pcl": "The infomation of all the objects in the scene:"}, 
    "eobj": {"image": "</Img>", "pcl": ".\n"},
    "simg": {"image": "</Img>", "pcl": "Multi-view images of the scene:"},
    "eimg": {"image": "</Img>", "pcl": ".\n"},
}


class MITStoppingCriteria(StoppingCriteria):
...
def build_one_instance(tokenizer, conversation, vision_type="pcl", template=conversations.default_conversation):
...
def make_prompt_start(use_system=False, vision_type="pcl", task_type="normal", template=conversations.default_conversation):
...
def make_prompt_middle(use_system=False, vision_type="pcl", task_type="normal", template=conversations.default_conversation):
    """make starting prompt
    ...
    """
    # PROMPT_START = f'### Human: {VISION_TAGS["sov"][vision_type]}'
    PROMPT_MIDDLE = f'{VISION_TAGS["eov"][vision_type]} {VISION_TAGS["sobj"][vision_type]}'
    return PROMPT_MIDDLE


def make_prompt_middle_img(use_system=False, vision_type="pcl", task_type="normal", template=conversations.default_conversation):
    """make starting prompt
    ...
    """
    # PROMPT_START = f'### Human: {VISION_TAGS["sov"][vision_type]}'
    PROMPT_MIDDLE = f'{VISION_TAGS["eobj"][vision_type]} {VISION_TAGS["simg"][vision_type]}'
    return PROMPT_MIDDLE

class 3DMITPEFTModel(nn.Module):


    def __init__(self, **args):
        super(3DMITPEFTModel, self).__init__()
        self.args = args
        self.input_dim =512
        ...
        self.visual_encoder_img = clip_encoder.visual
        if self.vision_feature_type == "global":  # global feature from CLIP
            ...
            self.num_vision_token_img = 4
        
        print('visual_encoder_img is loading!')
        if self.encoder_pretrain.lower() == "epcl":
            # PCL data Processing
            self.use_color = (
        ...
        print('visual_encoder_epcl is loading!')

        self.color_size_proj = nn.Linear(6, self.input_dim)
        ...
        for name, param in self.visual_encoder_img.named_parameters():
            param.requires_grad = False
        self.visual_encoder_img.eval()
        ...
        self.visual_encoder_img.to(self.llama_model.dtype)
    ...
    def encode_multi_view(self, vision_paths):


        all_scene_image_path='/data/GROUP_frontier/datasets/scannet/frames_square'
        pattern = re.compile(r'scene\d{4}_\d{2}')

        target_id = pattern.search(input_path[0]).group()
        target_scene=all_scene_image_path+'/'+target_id+'/color' 
        img_num=os.listdir(target_scene) #the list of all the image in the target scene  ['960.jpg', '1060.jpg', ...]
        # sample_num=16
        sample_num=64
        ls=[]
        new_img=[]
        
        if len(img_num)<sample_num+1:
            for img in img_num:
                img_path=target_scene+'/'+img
                vision_embeds, _ = self.encode_image(img_path)  #[bsz,4,4096]
                ls.append(vision_embeds)

            multi_img_token=torch.cat(ls, dim=1) #[1,4*img_num,4096]
            multi_img_token.to(self.device)
        elif len(img_num)>sample_num:
            
            for i in img_num:  # e.g. 60
                name=i[:-4]  
                new_img.append(int(name))  #e.g.[0, 20, 40, ...]
            ls1=sorted(new_img)
            ls2=[]
            for i in ls1:
                ls2.append(str(i)+'.jpg')   #['0.jpg', '20.jpg', '40.jpg', ...]
            
            # k= len(new_img)//sample_num  # 60//16 =3 
            # sampled_images = new_img[::k][:16]  #['0.jpg', '60.jpg', '120.jpg', ...]
            k= len(img_num)//sample_num  # 60//16 =3 
            sampled_images = ls2[::k][:sample_num]  #['0.jpg', '60.jpg', '120.jpg', ...]
            ...
            multi_img_token=torch.cat(ls, dim=1) #[1,4*16,4096]
            multi_img_token.to(self.device)
        else:
            raise NotImplementedError(
                "{} not Implemented".format(vision_paths)
            )


            
        return multi_img_token
    ...
    def encode_image(self, image_paths):
        ...
        inputs=inputs.half()
        inputs_llama = self.clip_encode_image(inputs) #[bsz,4,4096]
        atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(
            self.device
        )  # bsz x 1/256
        return inputs_llama, atts_llama  #[bsz,4,4096]
        # else:
    ...
    def encode_image_object(self, images):
        ...
        if self.encoder_pretrain == "clip":
            ...
            inputs_llama = self.clip_encode_image(inputs)  # bsz x 4 x llama_size
            atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(
                self.device
            )  # bsz x 1/256
    ...
    def encode_scnanet_object(self,input_path):
        '''
        obj: extract by ulip2 [bs, 512]
        attr: contains color:R,G,B; locs: x,y,z,l,w,h; [bs,9]
        ...
        # for i in object_all.keys():
        #     j=0
        #     if i.startswith(target_id):

        #         batch_size = 1
        #         #object_all[i] -> (512)
        #         feat=object_all[i].unsqueeze(0)
        #         feat=torch.unsqueeze(feat, 0).expand(batch_size, -1, -1)
        #         pos_emb = self.pos_embedding(torch.tensor(attr[target_ids]['locs'][j]), self.input_dim).permute(0, 2, 1)  # torch.Size([b, t, 512])
        #         size_color_emb = self.color_size_proj(torch.tensor(attr[target_id]['colors'][j]))  
        #         size_color_emb=size_color_emb.unsqueeze(0)            
        #         size_color_emb=torch.unsqueeze(size_color_emb, 0).expand(batch_size, -1, -1)                              # # torch.Size([b, t, 512])
        #         feat = self.input_norm(feat + size_color_emb + pos_emb)
        #         obj_feat = self.scene_proj(feat)  # torch.Size([b, t, 4096])


        #         ls.append(obj_feat)
        #     j+=1

        # obj_feat=torch.cat(ls, dim=1)
        # return obj_feat

        obj_path= './src/feature/scannet_ulip2_feats.pt'
        # obj_path='./src/feature/scannet_uni3d_feats_1024.pt'

        attr_path= './src/feature/scannet_attributes.json' 
       
        #batch_size_final = len(input_path)
        with open(attr_path, 'r') as file:
            attr = json.load(file)
        #bsz_obj_feat=[]
        #for batch in range(batch_size_final):
        pattern = re.compile(r'scene\d{4}_\d{2}')
        ...
        for i in object_all.keys():
            ...
            if i.startswith(target_id):
                ...
                batch_size = 1
                #object_all[i] -> (512)
                feat=object_all[i].unsqueeze(0).to(self.device)
                feat=feat.to(torch.half)
                feat=torch.unsqueeze(feat, 0).expand(batch_size, -1, -1)
                
                color_size=torch.tensor(attr[target_id]['colors'][j][0:3]+attr[target_id]['locs'][j][3:6]).to(self.device)  
                color_size=color_size.to(torch.half) 
                size_color_emb = self.color_size_proj(color_size)
                #change in 23:04 11/29
                            
                size_color_emb=size_color_emb.unsqueeze(0)            
                size_color_emb=torch.unsqueeze(size_color_emb, 0).expand(batch_size, -1, -1)  
                
                pos_dim3=torch.tensor(attr[target_id]['locs'][j][0:3]).to(self.device) #tensor([512])
                pos_dim3=pos_dim3.to(torch.half)  
                #pos_dim3.unsqueeze(0) #tensor([1,512])
                pos_dim3=torch.unsqueeze(pos_dim3, 0).expand(batch_size, -1, -1)  
                pos_emb = self.pos_embedding(pos_dim3, self.input_dim).permute(0, 2, 1)  # torch.Size([b, t, 512])
                #print(pos_emb.shape)          # # torch.Size([b, t, 512])
                feat_all = self.input_norm(feat + size_color_emb + pos_emb).to(self.device)
                obj_feat = self.scene_proj(feat_all).to(self.device)  # torch.Size([b, t, 4096])


                ls.append(obj_feat)
            j+=1
        
        obj_feat=torch.cat(ls, dim=1) #[1,num_obj,4096]
        obj_feat.to(self.device)
        return obj_feat

        #     target_size = 131
        #     padded_obj_feat = F.pad(obj_feat, (0, 0, 0, target_size- obj_feat.size(1)))

        #     bsz_obj_feat.append(padded_obj_feat)
        
        
        # obj_feat_final=torch.cat(bsz_obj_feat, dim=0)

        # return obj_feat_final   #[bsz,131,4096]
    ...
    def clip_encode_image(self, inputs):
        ...
        if self.vision_feature_type == "global":
            with torch.no_grad():
                embeddings = self.visual_encoder_img(inputs)  # bsz x 768
            ...
            with torch.no_grad():
                embeddings = self.visual_encoder_img.forward_patch_features(inputs)[
                    :, : self.num_vision_token_img
                ]  # bsz x self.num_vision_token x 1024
            image_embeds = embeddings.reshape(-1, self.vision_hidden_size).to(
                self.llama_model.dtype
            )  # bsz*num vision token x 1024
            inputs_llama = self.llama_proj(image_embeds).reshape(
                -1, self.num_vision_token_img, self.llama_model.config.hidden_size
            )  # bsz x num_vision_token_img x llama_size
        else:
            raise NotImplementedError(
    ...
    def load_and_transform_image_data_clip(self, image_path, device):
        ...
        image_output = self.visual_preprocess(image1).to(device)  # 3 x 224 x 224
    ...
    def prompt_wrap(
        self, img_embeds, obj_feat, multi_img_token, input_ids, target_ids, attention_mask, use_system, task_type
    ):
        """
        ...
        p_middle = make_prompt_middle(
            use_system=use_system, vision_type=self.vision_type, task_type=task_type, template=self.conv_template
        )

        p_middle_img=make_prompt_middle_img(
            use_system=use_system, vision_type=self.vision_type, task_type=task_type, template=self.conv_template
        )
        ...
        if isinstance(p_middle, list):
            ...
            # TODO: test in batch
            p_middle_token_ids = rnn.pad_sequence(
                p_middle_tokens,
                batch_first=True,
                padding_value=self.llama_tokenizer.pad_token_id,
            )  # bsz x s1
            p_middle_attn_mask = p_middle_token_ids.ne(
                self.llama_tokenizer.pad_token_id
            )
        else:
            ...
            p_middle_token_ids = p_middle_tokens.input_ids.expand(
                batch_size, -1
            )  # bsz x s1
            p_middle_attn_mask = p_middle_tokens.attention_mask.expand(
                batch_size, -1
            )  # bsz x s1
        # peft model need deeper call
        ...
        if isinstance(p_middle_img, list):
            p_middle_img_tokens = [
                self.llama_tokenizer(p, return_tensors="pt", add_special_tokens=False)
                .input_ids[0]
                .to(self.device)
                for p in p_middle_img
            ]
            # TODO: test in batch
            p_middle_img_token_ids = rnn.pad_sequence(
                p_middle_img_tokens,
                batch_first=True,
                padding_value=self.llama_tokenizer.pad_token_id,
            )  # bsz x s1
            p_middle_img_attn_mask = p_middle_img_token_ids.ne(
                self.llama_tokenizer.pad_token_id
            )
        else:
            p_middle_img_tokens = self.llama_tokenizer(
                p_middle_img, return_tensors="pt", add_special_tokens=False
            ).to(
                self.device
            )  # [s1, s1...] list of batch size
            p_middle_img_token_ids = p_middle_img_tokens.input_ids.expand(
                batch_size, -1
            )  # bsz x s1
            p_middle_img_attn_mask = p_middle_img_tokens.attention_mask.expand(
                batch_size, -1
            )  # bsz x s1
        # peft model need deeper call
        p_middle_img_embeds = self.llama_model.model.model.embed_tokens(
            p_middle_img_token_ids
        )  # .expand(batch_size, -1, -1) # bsz x s1 x embed_dim
        ...
        inputs_embeds = torch.cat(
            [bos_embeds, p_before_embeds, img_embeds,p_middle_embeds, obj_feat, p_middle_img_embeds, multi_img_token, p_after_embeds], dim=1
        )  # bsz x (1+ s1+NumToken+obj+s2) x embed_dim  torch.Size([1, 298, 4096])
           #<obj1>:  <obj2>:  ..... <obj44>:
        # make target ids for prefix part
        #import ipdb; ipdb.set_trace()
        empty_targets = (
            torch.ones(
                [batch_size, 1 + p_before_embeds.size()[1] + obj_feat.size()[1]+p_middle_embeds.size()[1]+ p_middle_img_embeds.size()[1]+ multi_img_token.size()[1]+ self.num_vision_token],
                dtype=torch.long,
            )
            .to(self.device)
        ...
        )  # bsz x (1 + s1 + obj+ num_image_tokens + s2)
        ...
        atts_obj = torch.ones([batch_size, obj_feat.size()[1]], dtype=torch.long).to(
            self.device
        )  # bsz x obj
        atts_multi_img_token=torch.ones([batch_size, multi_img_token.size()[1]], dtype=torch.long).to(
            self.device
        )  # bsz x multi_img_token
        atts_p_middle_img=torch.ones([batch_size, p_middle_img_embeds.size()[1]], dtype=torch.long).to(
            self.device
        )  # bsz x p_middle_img_embeds

        attention_mask = torch.cat(
            [atts_bos, p_before_attn_mask,atts_obj, 
            p_middle_attn_mask, atts_img, atts_multi_img_token,  p_middle_img_attn_mask ,attention_mask], dim=1
        )
        assert (
    ...
    def forward(self, inputs):
        ...
        multi_img_token=self.encode_multi_view(vision_paths)
        obj_feat=self.encode_scnanet_object(vision_paths)
    ...
    def prepare_generation_embedding(self, inputs):
        ...
        vision_paths = inputs["pcl_paths"]

        obj_feat=self.encode_scnanet_object(vision_paths)
        multi_view=self.encode_multi_view(vision_paths)
        ...
        p_middle_embeds = self.llama_model.model.embed_tokens(
        ...
        p_middle_img_embeds = self.llama_model.model.embed_tokens(
            p_middle_img_tokens.input_ids
        ).expand(
            batch_size, -1, -1
        )  # bsz x s1 x embed_dim
        ...
        inputs_embeds = torch.cat(
            [bos_embeds, p_before_embeds, feature_embeds, p_middle_embeds, obj_feat, p_middle_img_embeds, multi_view, p_after_embeds], dim=1
    ...
    def generate(self, inputs):
        ...
        stopping_criteria = StoppingCriteriaList(
            [MITStoppingCriteria([[2277, 29937], [835], [1, 2]], input_embeds)]            # TODO: different template has corresponding end signal [sep2]
        )
