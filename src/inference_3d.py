import os
from model.3dmit import 3DMITPEFT
import torch
import json
import argparse
...
def vqa_v1_val_response(args,
    ...
    reasoning_list = default_response(args, model, input, pcl_paths, sys_msg)
    all_answer_list = []
    for reasoning in zip(reasoning_list):
        all_answer_list.append(reasoning)
    return all_answer_list
...
def main(args):
    ...
    for data_item in tqdm(dataloader):
        ...
        if task_name == 'VQA':
            if dataset_name == 'ScanQA_v1.0_val_3dmit':
                response_func = vqa_v1_val_response
            else:
